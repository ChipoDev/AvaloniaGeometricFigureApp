...
namespace ShapesApp.Models
{
        ...
        public override (double Left, double Top, double Right, double Bottom) BoundingBox
        {
            get
            {
                ...
                for (int i = 1; i < VerticesX.Length; i++)
                {
                    ...
                    if (VerticesY[i] > top) top = VerticesY[i]; // Изменено на Max
                    if (VerticesY[i] < bottom) bottom = VerticesY[i]; // Изменено на Min
                }
    ...
    public class Ellipse : Shape
    {
        ...
        public override (double Left, double Top, double Right, double Bottom) BoundingBox
        {
            get
            {
                return (X - RadiusX, Y + RadiusY, X + RadiusX, Y - RadiusY); // Изменено для корректного порядка
            }
        }
